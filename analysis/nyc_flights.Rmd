...
Flights in January going to Miami 
```{r}
df4 <- df1 |>
...
There were 981 flights going to Miami in the month of January. 
 ...
 Flights in summer going to Chicago
...
# Which NYC airport had the most traffic in 2013?
...
# Bar Chart of number of flights

```{r}
df7 |>
  ggplot(aes(x = origin, y = n)) + geom_col() + labs(x = "Airport", y = "Number of Flights Out", title = "Flights out of NYC Airports in 2013") 
```
From this chart, we can see that EWR had the most flights out in 2013. 

# What was the top airline for each NYC airport in 2013?
...
df8 = df1 |>
  ...
  ggplot(df8, aes(x = dest, y = n) ) + geom_col(aes(fill = origin)) +theme(axis.text.x = element_text(angle = 90, vjust = .1, hjust = 1), axis.text = element_text(size = 6))
```
From this graph, we can see LAX was the most popular place for JFK flights. 
...
df8 = df1 |>
  ...
  ggplot(df8, aes(x = dest, y = n) ) + geom_col(aes(fill = origin)) +theme(axis.text.x = element_text(angle = 90, vjust = .1, hjust = 1), axis.text = element_text(size = 6))
```
From this graph, we can see ORD was the most popular place for EWR flights. 
...
df8 = df1 |>
  ...
  ggplot(df8, aes(x = dest, y = n) ) + geom_col(aes(fill = origin)) +theme(axis.text.x = element_text(angle = 90, vjust = .1, hjust = 1), axis.text = element_text(size = 6))
```
From this graph, we can see ATL was the most popular place for LGA flights. 
